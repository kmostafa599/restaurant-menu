...
export default function Header({ setIsOpen }) {
  ...
  return (
    <div className={`flex  ${location.pathname === '/store' ? 'justify-between' : 'justify-end'} md:border-r-2`}>
...
/>
    ...
    >
     Helllooooo
      ...
      {location.pathname === '/store' ?
        <div>
          <Link to='/' style={{ textDecoration: 'none' }}>
            <button className='rounded pointer w-[40px] h-[40px] m-3 mt-3 p-3 shadow'>
      ...
      <button className='  rounded pointer w-[40px] h-[40px] m-3 mt-3 p-3 shadow ' onClick={() => setIsOpen(true)}
      >
