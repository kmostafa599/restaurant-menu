import React, { useEffect, useState } from 'react'
import TextField from '@mui/material/TextField'
import { Link, useParams } from 'react-router-dom'

const ProductDetails = () => {
    //filter product by id 
    const { id } = useParams()

    //product counter
    let [counter, setCounter] = useState(1)
    let [editing, setEditing] = useState(true)

    // form values   
    const initialState = {
        counter:2
    }
    let [productDetails, setProductDetails] = useState(initialState)
    
    //handle product count
    const handleCount = (value) => {
        if (productDetails.counter <= 1 && value === -1) {
            return
        }
        setProductDetails({...productDetails,counter:value})
    }
    const handleSubmit = (e) =>{
        e.preventDefault()
        console.log('submitted')
    }

    const handleChange = (event)=>{
        const {name, value} = event.target
        console.log(event.target.ariaValueNow)
        setProductDetails({...productDetails,[name]:value})
    }
    console.log("product id: ", id)
    return (
        <div className='flex flex-col justify-center items-center space-y-2 w-full' >
            <div className='w-full h-60'>
                <Link to='/store'>
                    <button className=' z-10 absolute top-5 m-3 bg-white p-3 shadow rounded  '>
                        <svg className='rotate-180 text-gray-800 ' width="16" height="16" viewBox="0 0 24 26" fill="currentColor"  ><path d="M6.92 1c.255 0 .509.138.636.275l10.158 11.157c.381.413.381.964 0 1.377L7.683 24.69c-.381.413-.89.413-1.27 0-.381-.413-.381-.964 0-1.377l9.396-10.192L6.287 2.652c-.381-.413-.381-.964 0-1.378A.932.932 0 016.92 1z" fill="null" stroke="null" stroke-width="0.25"></path>
                        </svg>
                    </button>
                </Link>
                <img className='w-full h-full' src="https://img.freepik.com/free-photo/juicy-american-burger-hamburger-cheeseburger-with-two-beef-patties-with-sauce-basked-black-space_124865-5964.jpg?w=740" alt="" /></div>
            <div className='text-3xl font-bold'>Egg benedict burger</div>
            <div className='flex space-x-2'>
                <div className='text-[#F56565]'>$16</div> <span>&#8729;</span>
                <div>220 calories</div> <span>&#8729;</span>
                <div className='flex space-x-1'><svg className='align-self-center  ' width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path fill-rule="evenodd" clip-rule="evenodd" d="M.132 12c0 6.555 5.313 11.868 11.868 11.868 6.555 0 11.868-5.313 11.868-11.868C23.868 5.445 18.555.132 12 .132 5.445.132.132 5.445.132 12zm22.787 0c0 6.03-4.889 10.918-10.919 10.918S1.081 18.03 1.081 12C1.081 5.97 5.97 1.08 12 1.08S22.919 5.97 22.919 12zM6.303 13.424c0-.262.213-.474.475-.474H12V4.88a.475.475 0 11.95 0v9.019H6.778a.475.475 0 01-.475-.475z" fill="null"></path><path fill-rule="evenodd" clip-rule="evenodd" d="M12 .264C5.518.264.264 5.518.264 12c0 6.482 5.254 11.736 11.736 11.736 6.482 0 11.736-5.254 11.736-11.736C23.736 5.518 18.482.264 12 .264zM0 12C0 5.373 5.373 0 12 0s12 5.373 12 12-5.373 12-12 12S0 18.627 0 12zm.95 0C.95 5.897 5.896.95 12 .95 18.103.95 23.05 5.896 23.05 12c0 6.103-4.947 11.05-11.05 11.05C5.897 23.05.95 18.103.95 12zM12 1.213C6.043 1.213 1.213 6.043 1.213 12c0 5.957 4.83 10.787 10.787 10.787 5.957 0 10.787-4.83 10.787-10.787 0-5.958-4.83-10.787-10.787-10.787zm.475 3.323a.343.343 0 00-.343.343v8.202H6.778a.343.343 0 000 .686h6.04V4.879a.343.343 0 00-.343-.343zm.606 9.495V4.879a.607.607 0 00-1.213 0v7.939h-5.09a.606.606 0 100 1.213h6.303z" fill="null"></path></svg>
                    <span > 8 minutes</span></div>
            </div>
            <div className='px-3 text-center'>Lorem ipsum dolor sit amet consectetur adipisicing elit. Veritatis eligendi cumque consectetur nostrum maiores amet est et, sed</div>
            <div className='font-bold'>Choose size</div>

            {/* <form onChange={handleChange}onSubmit={handleSubmit}> */}
            <div className=' divide-y divide-solid w-full space-y-4  px-4'>
                <div className='pt-2'><input type="checkbox"  className='w-6 h-6 rounded-full text-[#F56565] border-[#F56565] focus:ring-red-200 ' /> 100 gr.</div>
                <div className='pt-2'><input type="checkbox" className='w-6 h-6 rounded-full text-[#F56565] border-[#F56565] focus:ring-red-200 ' /> 120 gr.</div>
                <div className='pt-2'><input type="checkbox" className='w-6 h-6 rounded-full text-[#F56565] border-[#F56565] focus:ring-red-200 ' /> 150 gr.</div>
            </div>

            <div className='font-bold'>Add a Note</div>
            <div className='w-full px-3 '><textarea className='border-2 w-full rounded-xl ' type="email" /></div>
            <div className='flex space-x-2 justify-center items-center text-center' >
                <button className='w-[40px] h-[40px] bg-[#F56565] rounded-full text-white hover:bg-[#d15f5f]' onClick={() =>{ handleCount(-1);setEditing(false)}} >-</button>
                <div>
                    {editing ?
                        (<div>
                            
                            <input type="counter" maxLength='2' id='counter' name='counter' value={productDetails.counter} onChange={handleChange} className='flex justify-center w-[40px] h-[40px] '  />
                        </div>
                        )
                        : (
                            <button onClick={() =>setEditing(true) }>
                                {productDetails.counter}
                            </button>
                        )

                    }


                </div>
                <button className='w-[40px] h-[40px] bg-[#F56565] rounded-full text-white hover:bg-[#d15f5f]' onClick={() => {handleCount(+1);setEditing(false)}}>+</button>
            </div>
            <div className='w-full h-12'>
                <button className='w-full h-full bg-[#F56565]' type='submit'>ADD TO CART</button>
            </div>
            {/* </form> */}
        </div>
    )
}

export default ProductDetails